...
namespace Pet.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        public IActionResult Index(string returnUrl = null)
        {
            ...
            ViewData["lsTinhThanh"] = new SelectList(_context.Location.Where(x => x.Levels == 1).OrderBy(x => x.Type).ToList(), "Location", "Name");
        ...
        public IActionResult Index(MuaHangVM muaHang)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    donhang.TransactionId = 1;//Don hang moi
        ...
        public string GetNameLocation(int idlocation)
        {
            try
            {
                var location = _context.Location.AsNoTracking().SingleOrDefault(x => x.LocationId == idlocation);
                if (location != null)
                {
